/*
Copy From https://github.com/denoland/deno_core/blob/0.199.0/core/module_specifier.rs
*/
...
mod tests {
  ...
  fn test_resolve_url_or_path_deprecated_error() {
    ...
    for (specifier, expected_err) in tests {
      let err = resolve_url_or_path(specifier, &PathBuf::from("/")).unwrap_err();
      assert_eq!(err, expected_err);
    }
  }
  ...
  //   let actual: ModuleSpecifier = from_value(json!("http://deno.land/x/mod.ts")).unwrap();
